...
- 本地 Chromium 全回归：29 项通过、4 项额外付费场景跳过。
  最后复测中一项截图写入遇到 Windows 文件错误，该文件的两个完整用例重跑通过；首次失败保留在私人验收日志。
- H5/weapp 连续构建通过，入口和主包预算检查通过。官方编译器通过 9 个 WXSS 文件。
- OSS 变更关联 H5 回归 6 项通过，1 项额外付费结果测试未启用；实际检查图片解码、五套主题、角色切换、账号安全和图谱。
- 官方 CLI 成功上传开发版本 `2026.9.16.1`，总包 634465 B、主包 542006 B。包内媒体为零，JS 压缩和 requiredComponents 注入开启；20 个 JS 文件通过 ES2019 解析。
