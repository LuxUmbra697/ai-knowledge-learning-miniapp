...
- 锁定 Python 3.13 环境：446 项离线通过、83 项 MySQL 通过，30 项前端单元通过。
- 最新布局版本：本地 Chromium 完整回归 34 项通过、4 项额外付费场景跳过，未新增模型调用。
- H5/weapp 连续构建通过，入口和主包预算检查通过。官方编译器通过 9 个 WXSS 文件。
- OSS 变更关联 H5 回归 6 项通过，1 项额外付费结果测试未启用；实际检查图片解码、五套主题、角色切换、账号安全和图谱。
- 官方 CLI 成功上传开发版本 `2026.9.16.2`，总包 635409 B、主包 542950 B。包内媒体为零，JS 压缩和 requiredComponents 注入开启；20 个 JS 文件通过 ES2019 解析。
...
开发工具实际完成账号注册/恢复/登录；另行通过真实微信 code 交换和官方开发版二维码，
小程序明确确认后 H5 登录成功。仅扫码导航由自动化注入 scene，未模拟身份接口，未验证真机摄像头扫码。

## 布局与可读性

`frontend/e2e/layout.spec.ts` 使用真实本地 API、隔离 MySQL 账号、合成长标题与已提交答案，
检查全部 14 个路由、五套主题及常用分页/弹层/角色状态。默认 CI 检查 390/1440 px；
本机扩展检查 320/768/1920 px。共 865 个页面与滚动状态，逐个保存实际截图及测量值。
普通启用文字的计算对比度至少 4.5:1，大字至少 3:1；另外检查按钮文字裁切、横向溢出和伙伴遮字。
禁用控件、图片背景标题和图谱内部不冒充完整 WCAG 认证；它们另有交互测试或截图检查。

本次先复现再修复：Taro H5 的 `disabled="false"` 误中框架禁用颜色、森林主题辅助文字偏淡、
伙伴遮挡题型标签/趋势数字/身份状态，以及微信数字输入框的垂直内边距。
账号提交中的禁用锁仍会阻止重复请求。首页合并重复工具区，保留上传、计划与练习入口。
原生开发工具逐页检查 14 页，并复验三个登录选项、角色切换与六个数量输入框；不是移动真机结论。
