...
- 锁定 Python 3.13 环境：449 项离线通过、84 项 MySQL 通过，33 项前端单元通过。
- 游客与账号修复版本：本地 Chromium 完整回归 38 项通过、5 项额外付费场景跳过，未新增模型调用。
- H5/weapp 连续构建通过，入口和主包预算检查通过。官方编译器通过 9 个 WXSS 文件。
- OSS 变更关联 H5 回归 6 项通过，1 项额外付费结果测试未启用；实际检查图片解码、五套主题、角色切换、账号安全和图谱。
- 官方 CLI 成功上传修复开发版本 `2026.9.16.3`，总包 641803 B、主包 549198 B。包内媒体为零，JS 压缩和 requiredComponents 注入开启；21 个 JS 文件通过 ES2019 解析。开发版上传不代表新版后端已部署或审核通过。
- `asset-delivery.test.ts` 防止重新引入本地图片打包；后端 CSP 回归只允许指定 OSS 图片域名。开发工具实际解码 10 张远程背景/角色/图标，不等于真机或合法域名验收。
- OSS 最终完整 H5 回归为 29 项通过、4 项额外付费场景跳过；微信真实报告 Canvas 为 350x380，采样 309 种颜色，未新增模型调用。自动化查询使用实际节点 ID；原生选择器不能照搬浏览器标签选择器。
- 开发工具使用固定 AppID。修复版在隔离本地 API/MySQL 验证真实微信按钮登录、账号注册、恢复码找回和旧会话撤销；本地 HTTP 测试临时关闭域名校验，上传前恢复。合法域名和真机未验收。
...
### 账号与访客回归
账号回归由 `backend/integration/test_identity.py` 验证单次兑换、并发绑定、恢复码复用拒绝、
密码重置/登录竞态、换绑与旧会话撤销、大小写敏感微信身份、过期及取消。
`frontend/e2e/account-security.spec.ts` 实际注册、修改密码、退出、恢复、拒绝旧凭据并核对用户 ID 不变。
开发工具实际完成账号注册/恢复/登录；本次真实微信按钮执行 `wx.login`、本地后端交换和首页数据请求，均成功。
H5 当前仅账号方式，不暴露未获授权的扫码入口；旧版开发码联调不是当前正式扫码功能的证明。
新原生换绑接口检查当前用户、密码与微信归属；并发、其他账号占用和错误密码由离线/SQL 回归覆盖。

`frontend/e2e/guest-access.spec.ts` 检查游客首页无私人 API 请求、拒绝登录、公开隐私说明、
安全返回路径与真实账号登录后返回目标页。生产 basename 也在返回路径单元测试中覆盖。
`account-security.spec.ts` 用挂起请求复现超时，确认 30 秒后给出超时提示并解锁按钮；用户取消与超时区别处理。
发布缓存测试构造同长度、同修改时间的新旧 HTML，确保不错误返回 304；H5 JS/CSS 入口同时使用内容指纹。

2026-09-16 本次远端 SSH 握手与公网 HTTPS 连接失败，发生在任何远端写入之前。部署和旧站共存复验
尚未执行成功，不把历史公网测试或本地测试当作本次部署验收。

## 布局与可读性

`frontend/e2e/layout.spec.ts` 使用真实本地 API、隔离 MySQL 账号、合成长标题与已提交答案，
检查全部 15 个路由、五套主题及常用分页/弹层/角色状态。默认 CI 检查 390/1440 px；
本机扩展检查 320/768/1920 px。共 920 个页面与滚动状态，逐个保存实际截图及测量值。
普通启用文字的计算对比度至少 4.5:1，大字至少 3:1；另外检查按钮文字裁切、横向溢出和伙伴遮字。
禁用控件、图片背景标题和图谱内部不冒充完整 WCAG 认证；它们另有交互测试或截图检查。

已复现并回归：Taro H5 的 `disabled="false"` 误中框架禁用颜色、森林主题辅助文字偏淡、
伙伴遮挡题型标签/趋势数字/身份状态，以及微信数字输入框的垂直内边距。本次进一步显式指定原生按钮
和未选中登录选项的颜色，保护隐私正文不被伙伴遮挡，修复带 H5 basename 的登录返回路径。
账号提交中的禁用锁仍会阻止重复请求。首页合并重复工具区，保留上传、计划与练习入口。
本次原生开发工具逐页检查 15 页的就绪数据、控件边界与可见颜色，并复验三个登录选项、角色切换与六个数量输入框。
注入尚未授权的平台状态后，上传先显示说明，拒绝时不打开文件选择器。真实微信登录不使用身份 Mock。
截图须等远程图片解码；曾在图片加载前捕获到空白书架，补查真实尺寸与加载完成截图后排除布局问题。
这些检查不是移动真机、全页面原生对比度认证或生产合法域名结论。

扩展矩阵命令（已有隔离 API 与 H5 预览时，在 `frontend` 执行）：

```powershell
$env:LAYOUT_ALL_WIDTHS='1'
npx playwright test e2e/layout.spec.ts e2e/companion.spec.ts
```

```sh
LAYOUT_ALL_WIDTHS=1 npx playwright test e2e/layout.spec.ts e2e/companion.spec.ts
```

汇总见 [布局证据](evidence/layout-ui.json)。私人巡检截图、诊断日志与实施工件不上传。
