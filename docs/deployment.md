...
2026-09-17 已部署源码 `8451b3b`：包含此前游客、登录与隐私修复及实际生效的低日志量策略。
公网浏览器已验证不登录浏览、取消登录提示、可见登录/注册入口、账号登录、伙伴切换与原有问答/报告持久化。
三个站点入口及原 API、学习 API ready 正常；未知 API 返回 JSON 404。本轮没有新增模型调用。
本次只重建学习项目容器，网关配置、旧站容器 ID、业务环境文件和数据库结构未变。
对应提交的两组 CI 通过，含隔离 MySQL 5.7/8、双端构建与真实 H5 回归；小程序正式发布仍单独验收。
