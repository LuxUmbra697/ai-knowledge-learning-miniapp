...
这不会由普通 GitHub runner 代替。H5 使用账号注册/登录/恢复码；小程序另有真实微信 code 交换。
首页可直接浏览，使用个人功能时再询问登录。未登记微信可选注册、绑定已有账号或取消，不自动合并。
运营者还需完成 [微信后台隐私声明](wechat-privacy.md)，应用内说明不能替代平台配置。
