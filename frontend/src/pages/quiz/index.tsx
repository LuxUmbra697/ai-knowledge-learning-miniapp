  ...
  return <StudioShell title={quiz?.title || '知识练习'} subtitle='先独立思考，再与解析对照。' focus>
    ...
    <View className='practice-surface'>
      ...
      {question && <>
        ...
        {record && !record.is_correct && <Button className='text-button' onClick={() => Taro.navigateTo({ url: `/learning/tutor/index?quizId=${encodeURIComponent(quizId)}&questionId=${encodeURIComponent(question.id)}` })}><Icon name='chat' size={16} />分析这道错题</Button>}
