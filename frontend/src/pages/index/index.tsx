...
import { requireLogin, openPage } from '../../services/access'
...
export default function HomePage() {
  ...
  const [visitor, setVisitor] = useState(!getToken())
  ...
  const load = useCallback(async () => {
    alive.current = true
    await waitForLogin()
    if (!getToken()) { setVisitor(true); setProfile(null); setHistory([]); setDocumentCount(0); setLearning(null); setPending(null); setError(''); return }
    setVisitor(false)
    ...
    try {
      ...
      if (reason instanceof ApiError && reason.statusCode === 401) { setVisitor(true); setProfile(null); setHistory([]); setDocumentCount(0); setLearning(null); setPending(null); setError('') }
  ...
  const generate = async () => {
    if (!await requireLogin('/pages/index/index')) return
  ...
  return <StudioShell active='home' title='学习手帐' subtitle={visitor ? '今天想读些什么？' : `${profile?.nickname || getCachedUser()?.nickname || '同学'}，今天想读些什么？`}>
    {visitor && <View className='guest-browse'><Text className='muted'>访客浏览</Text><Button className='text-button' onClick={() => navigate('/pages/login/index')}>登录 / 注册</Button></View>}
    ...
    <View className='today-review section-heading'><View><Text className='section-title'>今日复习</Text><Text className='muted'>{visitor ? '登录后查看复习安排' : learning ? `${learning.due_count} 道到期 · 今日已复习 ${learning.today_reviews} 道` : '正在读取复习计划'}</Text></View><Button className='secondary-button' onClick={() => openPage('/learning/review/index')}><Icon name='review' size={17} />复习与掌握</Button></View>
    <View className='stats-row home-stats'><View className='stat'><Text className='muted'>我的知识文档</Text><Text className='stat-number'>{visitor ? '--' : documentCount}</Text><Text className='tiny-label'>篇学习材料</Text></View><View className='stat'><Text className='muted'>累计练习</Text><Text className='stat-number'>{visitor ? '--' : profile?.quiz_count || 0}</Text><Text className='tiny-label'>次探索与尝试</Text></View><View className='stat'><Text className='muted'>已完成正确率</Text><Text className='stat-number'>{visitor ? '--' : profile?.quiz_count ? `${profile.average_accuracy}%` : '暂无'}</Text><Text className='tiny-label'>{visitor ? '登录后记录' : `${profile?.total_xp || 0} 学习经验`}</Text></View></View>
    <View className='home-tools'><Button className='secondary-button' onClick={() => navigate('/pages/knowledge/index')}><Icon name='upload' size={18} />添加学习材料</Button><Button className='text-button' onClick={() => openPage('/learning/path/index')}><Icon name='review' size={17} />学习路径与计划</Button></View>
