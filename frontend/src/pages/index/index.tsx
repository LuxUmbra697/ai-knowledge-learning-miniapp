...
export default function HomePage() {
  ...
  const load = useCallback(async () => {
    ...
    setPending(saved)
    if (saved) { setInput(saved.input); setCounts(saved.counts); setWeb(saved.web) }
  ...
  const generate = async () => {
    ...
    try {
      let saved = restorableTopic(Taro.getStorageSync(key)) || { key: `topic_${Date.now()}_${Math.random().toString(36).slice(2)}`, input: input.trim(), counts, web }
      Taro.setStorageSync(key, saved); setPending(saved)
      if (!saved.taskId) {
        const { task_id } = await generateQuizAsync(saved.input, countQuestions(saved.counts), undefined, false, saved.key, saved.counts, saved.web)
