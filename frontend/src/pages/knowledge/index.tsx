...
import { getKnowledgeDocuments, getKnowledgeDocumentStatus, uploadKnowledgeDocument, deleteKnowledgeDocument, generateQuizAsync, pollQuizTask, waitForLogin, getToken, KnowledgeDocumentItem } from '../../services/api'
...
export default function KnowledgePage() {
  ...
  const upload = async () => {
    ...
    try {
      ...
      await uploadKnowledgeDocument(file.path, file.name)
      await load()
  ...
  return <StudioShell active='knowledge' title='我的知识书架' subtitle='让自己的学习材料，成为每次探索的起点。'>
    ...
    {documents.map(doc => <View className='document-row' key={doc.doc_id}><Icon name='book' size={26} /><View className='row-copy'><Text className='row-title'>{doc.file_name}</Text><Text className='muted'>{statusText[doc.status]} · {(doc.file_size / 1024).toFixed(1)} KB{doc.status === 'ready' ? ` · ${doc.chunk_count} 个片段` : ''}</Text>{doc.error_message && <Text className='muted'>{doc.error_message}</Text>}<View className='document-actions'>{doc.status === 'ready' && <Button className='secondary-button' disabled={!!busy} onClick={() => practice(doc)}>知识练习</Button>}<Button className='text-button' disabled={!!busy} onClick={() => remove(doc)}>删除文档</Button></View></View></View>)}
  </StudioShell>
}
